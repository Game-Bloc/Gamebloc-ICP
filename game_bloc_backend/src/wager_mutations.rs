...
pub fn expected_wager_reward(tournament_id: String, staker_principal_id: String) -> Vec<u128> {
    TOURNAMENT_STORE.with(|tournament_store| {
        let tournament = tournament_store
            ...
            .cloned()
            .unwrap();
        match tournament.wagers.to_owned() {
            None => {
                vec![]
            }
            Some(wagers) => {
                let staker_wagers: Vec<Wager> = wagers
                    .to_owned()
                    .iter()
                    .cloned()
                    .filter(|w| w.staker_principal_id == staker_principal_id)
                    .collect();
                let mut expected_rewards = vec![];
                for x in staker_wagers {
                    let first_player_staked_on: String = x.player_principal_id.to_owned();
                    let player_wagered_on: Vec<Wager> = wagers
                        .to_owned()
                        .iter()
                        .cloned()
                        .filter(|w| w.player_principal_id == first_player_staked_on)
                        .collect();
                    let wagers_on_other_players: Vec<Wager> = wagers
                        .to_owned()
                        .iter()
                        .cloned()
                        .filter(|w| w.player_principal_id != first_player_staked_on)
                    ...
                    for j in wagers_on_other_players {
                        total_amount_wagered_against_player_staked_on =
                            total_amount_wagered_against_player_staked_on + j.amount;
                    }
                    for i in player_wagered_on {
                        total_amount_wagered_on_player_staked_on =
                            total_amount_wagered_on_player_staked_on + i.amount;
                    }
                    for x in wagers.to_owned() {
...
pub fn get_wager(tournament_id: String, staker_principal_id: String) -> Option<Wager> {
    TOURNAMENT_STORE.with(|tournament_store| {
        ...
        match tournament.wagers.to_owned() {
            ...
            Some(wagers) => wagers
                .iter()
                .find(|&w| w.staker_principal_id == staker_principal_id)
                .cloned(),
        }
    })
}
...
pub fn add_or_increase_tournament_wager(id: String, wager: Wager) {
    TOURNAMENT_STORE.with(|tournament_store| {
        ...
        match tournament.wagers.to_owned() {
            None => {
                tournament.wagers = Some(vec![wager.to_owned()]);
            }
            Some(mut wagers) => {
                let exisiting_wagers: Vec<Wager> = wagers
                    .iter()
                    .cloned()
                    .filter(|w| w.staker_principal_id == wager.staker_principal_id)
                    .collect();
                let mut exisiting_wager = exisiting_wagers[0].to_owned();
                if exisiting_wagers.len() == 0 {
                    wagers.push(wager);
                    tournament.wagers = Some(wagers);
                } else {
                    let wager_index: Option<usize> = wagers
                        .iter()
                        .position(|w| w.staker_principal_id == wager.staker_principal_id);
...
pub fn get_all_wagers(id: String) -> Vec<Wager> {
    TOURNAMENT_STORE.with(|tournament_store| {
        let tournament = tournament_store.borrow().get(&id).cloned().unwrap();
        match tournament.wagers.to_owned() {
            None => {
                vec![]
            }
...
pub fn does_wager_exist(id: String, account_id: String) -> bool {
    TOURNAMENT_STORE.with(|tournament_store| {
        ...
        match tournament.wagers.to_owned() {
            ...
            Some(wagers) => wagers
                .iter()
                .find(|&w| w.staker_principal_id == account_id)
                .is_some(),
        }
    })
}
