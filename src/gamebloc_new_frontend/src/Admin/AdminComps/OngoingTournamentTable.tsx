...
const OngoingTournamentTable = () => {
  ...
  const ongoingTournament = data
    .filter(
      (tour: any) =>
        Object.keys(tour.status)[0].toUpperCase() === "GAMEINPROGRESS",
    )
    .map((tour: any) => tour)
  console.log("ongoingTournament", ongoingTournament)

  const dataSearch = ongoingTournament.filter((obj) => {
...
export default OngoingTournamentTable
