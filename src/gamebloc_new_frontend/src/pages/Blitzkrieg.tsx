...
const Blitzkrieg = () => {
  ...
  const pageCount: number = Math.ceil(
    blitzkriegTournament?.length / tournamentPerPage,
  )
  const changePage = ({ selected }: any) => {
    setPageNumber(selected)
  }
  const displayTournaments = blitzkriegTournament
    ?.slice(tournamentViewed, tournamentViewed + tournamentPerPage)
    .map((data: any, index: any) => (
      <TournamentCard data={data} index={index} key={index} />
    ))

  console.log(blitzkriegTournament?.length == 0)
